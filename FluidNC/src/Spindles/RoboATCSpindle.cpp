...
namespace Spindles {
    RoboATCSpindle::RoboATCSpindle() : VFD() {}

    void RoboATCSpindle::direction_command(SpindleState mode, ModbusCommand& data) {
        // NOTE: data length is excluding the CRC16 checksum.
        data.tx_length = 6;
        data.rx_length = 6;

        // data.msg[0] is omitted (modbus address is filled in later)
        data.msg[1] = 0x05; //write coil?
        data.msg[2] = 0x00;
    ...
    VFD::response_parser RoboATCSpindle::initialization_sequence(int index, ModbusCommand& data) {
        return nullptr;
        // NOTE: data length is excluding the CRC16 checksum.
        data.tx_length = 6;
        data.rx_length = 5;
        ...
        if (index == -1) {
            ...
            return [](const uint8_t* response, Spindles::VFD* vfd) -> bool {
                ...
                auto h100           = static_cast<RoboATCSpindle*>(vfd);
    ...
    VFD::response_parser RoboATCSpindle::get_current_speed(ModbusCommand& data) {
        // NOTE: data length is excluding the CRC16 checksum.
